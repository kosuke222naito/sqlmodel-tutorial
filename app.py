...
def update_heroes():
    with Session(engine) as session:
        ...
        print(f"Updated Spider-Boy's Teams: {hero_spider_boy.team_links}")
        print(f"Updated Z-Force's Heroes: {team_z_force.hero_links}")

        for link in hero_spider_boy.team_links:
            if link.team.name == "Preveneters":
        ...
        for link in hero_spider_boy.team_links:
            print(f"Spider-Boy teem: {link.team}, is_training: {link.is_training}")
